...
def get_batch_lyrics_async(authors_and_titles, not_founds):
    ...
    with Pool() as pool:
        all_lyrics = pool.starmap(get_lyrics, [song + (not_founds,) for song in authors_and_titles_list])
    return all_lyrics
...
def main():
    ...
    df = df.iloc[15:, 1:].reset_index().drop('index', axis=1).rename(
        {'Unnamed: 1': 'author', 'Unnamed: 2': 'title', 'Unnamed: 3': 'label'}, axis=1)
    ...
    for i, chunk in enumerate(dfs):
        ...
        lyrics = get_batch_lyrics_async(map(lambda r: (r[1]['author'], r[1]['title']), chunk.iterrows(), ), not_founds)
